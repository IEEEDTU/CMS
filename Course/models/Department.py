...
class DepartmentManager(models.Manager):
	def addDepartment(self, request):
		...
		D = Department(
			...
			hod = request['hod'],
			#hod = Faculty.objects.get(facultyId = request['facultyId']),
			email = request['email'],
	...
	def retrieveDepartments(self, request):
...
class Department(models.Model):
	...
	#hod = models.ForeignKey('Faculty', related_name="head_of_dept", on_delete = models.CASCADE, default=False)
	# Email address
